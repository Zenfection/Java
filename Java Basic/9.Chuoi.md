...
| codePointAt(*p*)<br>codePointBefore(*p*)<br>codePointCount(*a,b*) | trả về số bảng mã `Unicode` <br>trả về số bảng mã `Unicode`<br>trả về số lượng `Unicode` từ *a tới b* | int     | `myName.codePointAt(0)` ⇨ 32<br>`myName.codePointAt(1)` ⇨ 72<br>`myName.codePointCount(0,3)` ⇨ 3 | ⚠️ Sự khácnhau giữa `codePointAt` và `codePointBefore` là<br>`codePointAt` bắt đầu bằng *0*<br>`codePointBefore` bắt đầu bằng **1** |
