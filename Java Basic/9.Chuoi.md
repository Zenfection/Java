...
| codePointAt(*p*)<br>codePointBefore(*p*)<br>codePointCount(*a,b*) | trả về số bảng mã `Unicode` <br>trả về số bảng mã `Unicode`<br>trả về số lượng `Unicode` từ *a tới b* | int     | `myName.codePointAt(0)` ⇨ 72<br>`myName.codePointAt(1)` ⇨ 72<br>`myName.codePointCount(0,3)` ⇨ 3 | ⚠️ `codePointAt` bắt đầu bằng *0*<br>⚠️ `codePointBefore` bắt đầu bằng *1* |
| compareTo()<br>compareToIgnoreCase()                              | So sánh 2 chuỗi<br>So sánh 2 chuỗi không phân biệt *hoa thường*                                       | int     | `myName.compareTo("Hello")` ⇨ 0<br>`myName.compareToIgnoreCase("HELLO")` ⇨ 0                     | ⚠️ Nếu đúng sẽ trả về `0`, sai sẽ trả về khác `0`                          |
| concat()                                                          | nối 2 chuỗi                                                                                           | String  |                                                                                                  |                                                                            |
